...
class Gumroad
  def initialize(token)
    ...
    @start_page_url = "/v2/sales/?before=#{Date.today}&after=#{Date.today - 100}"
  end

  def sales_page_getter(page_url)
...
loop do
  sales_item_data = gumroad.sales_page_getter(sales_item_data['next_page_url'])
  sales_count += sales_item_data['sales'].size
